...
public class sketch_200725a extends PApplet {
...
public void generate(){
    ...
    dif = true;
    runtime = millis();
    int x = PApplet.parseInt((mouseX-54)/37);
    int y = PApplet.parseInt((mouseY-118)/40);
...
public int countFlags(Value[][] cubes){
   int num=0;
   for(int i=0;i<16;i++)
    for(int j=0;j<16;j++)
...
public int whatNumber(Value cube){
  int num = 1;
  for(int i=1;i<9;i++)
...
class Value{
   ...
   public int checkBombs(Value[][] cubes){
     ...
     for(int i=-1;i<=1;i++)
       for(int j=-1;j<=1;j++){
          int x = PApplet.parseInt(this.place.x) + j;
          int y = PApplet.parseInt(this.place.y) + i;
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "sketch_200725a" };
    if (passedArgs != null) {
